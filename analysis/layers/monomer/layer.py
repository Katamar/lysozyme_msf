...
def msd_block(selection, u1, u2, begin, timeframe, blocklen, temp):
    msd_bl=[0]*3
    tmpbegin=begin
    for i in selection:
        listi={}
    ...
    open("{0}_inner.dat".format(temp), 'a').write("%s\n" %(msd_bl[0]))
    open("{0}_middle.dat".format(temp), 'a').write("%s\n" %(msd_bl[1]))
    open("{0}_outer.dat".format(temp), 'a').write("%s\n" %(msd_bl[2]))
...
def main():
    ...
    for i in range(0, len(u2.trajectory)/blocklen*blocklen, blocklen):
        begin=i
        msd_block(selection, u1, u2, begin, timeframe, blocklen, temp)
